c Interpolate Sunshine Time data
c Include simple data check: the reasonable data range is 0.0 to 24.0
	subroutine Interpolate_sun(nr,nc,n_stn,startyear,endyear,
	...
	do i=1, days_fromstart(startyear,1,1,endyear,12,31)
		...
		do j=1, n_stn
			if (stn(j).nodata(i).ne.0) then
				if (stn(j).sun(i).ne.-99999.0) then
					npi=npi+1
					xi(npi)=stn(j).x
					yi(npi)=stn(j).y
					fi(npi)=stn(j).sun(i)
					if (fi(npi).lt.0.0.or.fi(npi).gt.24.0) then
					PRINT *,'INPUT ERROR.'
					PRINT *,'DAY:',i,'AT',xi(npi),yi(npi),'SUNSHINE:',fi(npi)
					STOP 'INPUT SUNSHINE TIME OUT OF RANGE.'
					endif
				endif
			endif	
		enddo
...
1004			call LOTPS (1,NPPR,npi,xi,yi,fi,nc,x,nr,y,IWK,NIWK,
	...
	enddo
	RETURN
1008	PRINT *, 'ERROR OCCUR IN INTERPOLATE!'
	RETURN
	end