c Interpolate Tmin data (Min Temperature)
c Include simple data check: the reasonable data range is -90 to +70
	subroutine Interpolate_tmin(nr,nc,n_stn,startyear,endyear,
	...
	do i=1, days_fromstart(startyear,1,1,endyear,12,31)
		...
		do j=1, n_stn
			if (stn(j).nodata(i).ne.0) then
				if (stn(j).tmin(i).ne.-99999.0) then
					npi=npi+1
					xi(npi)=stn(j).x
					yi(npi)=stn(j).y
					fi(npi)=stn(j).tmin(i)
					...
					PRINT *,'INPUT ERROR.'
					PRINT *,'DAY:',i,'AT',xi(npi),yi(npi),'Tmin:',fi(npi)
					STOP 'INPUT MIN TEMPERATURE OUT OF RANGE.'
					endif
				endif
			endif	
		enddo
...
1004			call LOTPS (1,NPPR,npi,xi,yi,fi,nc,x,nr,y,IWK,NIWK,
	...
	enddo
	RETURN
1003	PRINT *, 'ERROR OCCUR IN INTERPOLATE!'
	RETURN
	end