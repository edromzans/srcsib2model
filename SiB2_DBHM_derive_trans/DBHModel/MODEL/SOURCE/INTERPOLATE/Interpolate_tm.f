c Interpolate Tmean data (Mean Temperature)
c Include simple data check: the reasonable data range is -90 to +70
	subroutine Interpolate_tm(nr,nc,n_stn,startyear,endyear,
...
c		print *, 'tm at:',i,DATE_TIME
		...
		do j=1, n_stn
			if (stn(j).nodata(i).ne.0) then
				...
				PRINT *,'INPUT ERROR.'
				PRINT *,'DAY:',i,'AT',xi(npi),yi(npi),'Tmean:',fi(npi)
				STOP 'INPUT MEAN TEMPERATURE OUT OF RANGE.'
					endif
				endif
			endif	
		enddo
