c	Calculate hourly Evapoaration according to FAO_Penman_monteith method
...
c		calculate from input as,bs,alpha,r_a, r_s
	subroutine FAO_PMON_hour(Lat,Lon,Lon0,Alt,time,DJ,DN,as,bs,alpha,
     $	r_a,r_s,tm,tmax,tmin,um,n_summ,fsm,rsum,sun,ID,ET)
      implicit none
	Real	time			!standard clock time at the midpoint of the period 
	real	b,Sc,w,w1,w2,Lon,Lon0
	...
	PRINT *, "Subroutine FAO_PMON_hour cannot work now!"
	STOP
...
c	Calculate Extraterrestrial radiation for hourly periods (Ra) (E28)
	pi=atan(1.0)*4.0
	Gsc=0.0820
	dr=1.0+0.033*cos(2*pi*real(DJ)/real(DN))			!(E23)
	delta=0.406*sin(2*pi*real(DJ)/real(DN)-1.39)		!(E24)
	varphi=Lat*pi/180.0									!(E22)
	b =2.*pi*(real(DJ)-81.)/364.
	Sc=0.1645*sin(2*b) - 0.1255*cos(b) - 0.025*sin(b)	!(E32)
	w =pi/12.*((time + 0.06667*(Lon0-Lon)+Sc)-12.)		!(E31)
	w1=w - pi/24.										!(E29) hourly
	w2=w + pi/24.										!(E30) hourly
	Ra=12.0*60.0/pi*Gsc*dr*( (w2-w1)*sin(varphi)*sin(delta)+
     $cos(varphi)*cos(delta)*(sin(w2)-sin(w1)) )			!(E28)
...
c When calibrated values for as and bs are not available:
	...
	If (RsRso.gt.1.0) then
		...
		RsRso=1.0
	Endif
	Rnl=sigmaT*(0.34-0.14*sqrt(ea))*(1.35*RsRso-0.35)/24.	!(E39)
