c	This subroutine try to get trend of daily ReferET
...
c		RETa,RETb,RETr2: the array saves a,b,r2 of the period (before interpolated)
	Subroutine ANA_ReferET_TREND(StartY,
	...
	integer	nr,nc
	real	x0,y0,s
	real	ret(max_time,max_nr,max_nc)
	...
	write(YYYY,'(I4)') year
	write(WAY,'(I1)') INTPLT
	FName=NDVI_ANA(l1:l2)//"RET"//YYYY//"_"//WAY//".txt"
	open(11,file=FName,status='unknown')
	FName=NDVI_ANA(l1:l2)//"RET"//YYYY//"M_"//WAY//".txt"
	open(22,file=FName,status='unknown')
	FName=NDVI_ANA(l1:l2)//"RET"//YYYY//"Y_"//WAY//".txt"
	...
	DO While (DOIT.EQ.1) 
		ID1=ID1+1
		call Get_potential_ET(year,ret)
		NT=days_fromstart(
     $			year,1,1,year,12,31)
		NF=days_fromstart(
