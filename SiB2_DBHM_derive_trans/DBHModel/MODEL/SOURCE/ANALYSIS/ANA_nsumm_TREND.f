c	This subroutine analysis the Trend of Cloud cover
...
c		RETa,RETb,RETr2: the array saves a,b,r2 of the period (before interpolated)
	Subroutine ANA_nsumm_TREND(StartY,
	...
	write(YYYY,'(I4)') year
	write(WAY,'(I1)') INTPLT
	FName=NDVI_ANA(l1:l2)//"CLOUD"//YYYY//"_"//WAY//".txt"
	open(11,file=FName,status='unknown')
	FName=NDVI_ANA(l1:l2)//"CLOUD"//YYYY//"M_"//WAY//".txt"
	open(22,file=FName,status='unknown')
	FName=NDVI_ANA(l1:l2)//"CLOUD"//YYYY//"Y_"//WAY//".txt"
...
c			print *,NF,k,NF+k
			...
			do i=1,nr
			do j=1,nc
				ReferET(NF+k,i,j)=n_summ(k,i,j)
				aret(i,j)=n_summ(k,i,j)
			enddo
			enddo
