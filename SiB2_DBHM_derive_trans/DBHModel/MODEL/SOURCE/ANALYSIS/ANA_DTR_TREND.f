c	This subroutine analysis the Trend of DTR (diurnal temperature range)
...
c		RETa,RETb,RETr2: the array saves a,b,r2 of the period (before interpolated)
	Subroutine ANA_DTR_TREND(StartY,
	...
	write(YYYY,'(I4)') year
	write(WAY,'(I1)') INTPLT
	FName=NDVI_ANA(l1:l2)//"DTR"//YYYY//"_"//WAY//".txt"
	open(11,file=FName,status='unknown')
	FName=NDVI_ANA(l1:l2)//"DTR"//YYYY//"M_"//WAY//".txt"
	open(22,file=FName,status='unknown')
	FName=NDVI_ANA(l1:l2)//"DTR"//YYYY//"Y_"//WAY//".txt"
...
c			print *,NF,k,NF+k
			...
			do i=1,nr
			do j=1,nc
			if (tmax(k,i,j).ne.-9999.and.tmin(k,i,j).ne.-9999) then
				ReferET(NF+k,i,j)=max( 0.0, tmax(k,i,j)-tmin(k,i,j))
				aret(i,j)=max( 0.0, tmax(k,i,j)-tmin(k,i,j) )
			else
				ReferET(NF+k,i,j)=-9999
				aret(i,j)=-9999
			endif
			enddo
			enddo
